...
- `cy.intercept()` + `cy.wait('@alias')` only works in real-API mode. In mock mode MockHttpClient intercepts before the network — no network call is made, so the alias never resolves.
...
**Never call `cy.intercept()` directly for response stubs** in mock mode — it will not fire when MockHttpClient is active.

Low-level commands (mock mode only, must be called after `cy.visit()`):
- `cy.setMockError(path, error)` — programs `window.__mockClient__` to return an error
- `cy.setMockResponse(path, data)` — programs `window.__mockClient__` to return custom data

Default mock responses are set by `buildFeaturesMockClient()` (packages) and `buildAdminMockClient()` (admin app) on every page load. No manual reset is needed between tests.

**Switching modes** (controlled by `NEXT_PUBLIC_API_MOCKING` in `apps/admin/.env.local`):
- `enabled` → MockHttpClient active, no backend required
- unset / any other value → real HTTP, backend required
