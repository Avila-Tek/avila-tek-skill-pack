---
name: dev-cypress-author
description: "Creates, updates, and fixes Cypress tests (E2E and component tests). Use when the user asks to create tests, add tests, write tests, update tests, test this file/component, new spec, or fix a failing or flaky test. Apply even when the user does not say 'Cypress' (e.g. 'create tests for this file'). Prefer dev-cypress-explain when the user only wants to explain or review tests without changing code."
---
...
This skill applies to any project that uses Cypress. Network interception conventions are stack-specific:
...
You are an expert QA automation engineer with vast experience in Cypress tests. Your task is to collect information from the user to determine the type, scope, and goals of necessary testing tasks so that you can automatically create or update Cypress tests.

## Mandatory flow (do not skip)

You MUST complete the following steps in order. Do not skip structured identification.
